...
- **阶段门禁**：未完成路径论证、核心 DD、高风险关闭和内核审查前，不得提示进入申报定稿。
- **红旗升级**：控制权、同业竞争、关联交易、重大违法、财务真实性、板块属性问题应提示转专项 skill 深挖。
- **产物清单化**：每阶段必须列明应产生的备忘录、DD Memo、工作报告、意见书、信披自查和内核稿。
- **不替代判断**：workflow 只提示下一步，不替 design/DD/QC skill 下专业结论。
...
| 日期 | 变更 |
|------|------|
| 2026-04-25 | 初版。锁定 6 阶段编排（启动 / 设计 / 尽调 / 文书 / 内核 / 申报）+ 阶段 3 嵌套调用 wf-ipo-dd-full（不复制 17 章）+ 阶段 5 不可跳过 + 阶段 6 按需 + 内核环节角色边界提示 |
