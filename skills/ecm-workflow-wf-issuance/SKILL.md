...
- **发行条件门禁**：财务、规范运作、资金占用、违规担保、处罚、前募使用和募投合规未核清前，不进入文书定稿。
- **工具差异**：定增、配股、可转债、优先股的发行条件、会议程序、信披文件和审核重点必须分别列示。
- **第18号适用意见敏感**：再融资负面清单和发行条件判断必须提示核验最新适用意见。
- **组合交易**：再融资伴随并购重组时，应提示并行触发 `wf-ma-full` 而非混成一个流程。
...
| 日期 | 变更 |
|------|------|
| 2026-04-25 | 初版。锁定 6 阶段编排 + 阶段 2 强制工具比选（5 类）+ 阶段 3 上市公司视角的 13 章必查子集（不查 establishment / tax / environment / independence）+ 阶段 4 三项硬规则（特别决议 + 关联股东回避 + 中小投资者单独计票）+ IPO 红线再核查（关联交易 / 资金占用 / 违规担保） |
