...
- **内核独立性**：不得替起草人润色掩盖问题；发现事实冲突和重大遗漏必须用批注提出，不直接替换为未经确认事实。
- **三向比对**：信披文件、DD Memo/工作报告、审计/评估/财务顾问文件之间必须做硬字段和结论比对。
- **法规时效审查**：发行、再融资、重组、治理、信披规则引用必须核验版本和施行日。
- **高风险触发器**：风险因素弱化、重大违法遗漏、关联交易披露不完整、财务法律口径冲突，应列必改。
