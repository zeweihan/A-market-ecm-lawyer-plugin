...
- **交易类型先判定**：一般并购、重大资产重组、重组上市、上市公司收购、控制权交易必须先分类。
- **上市公司义务**：涉及上市公司时，信披、停复牌、内幕信息、董事会/股东会程序和中小投资者保护必须进流程。
- **标的 DD 定向**：按股权收购、资产收购、发股购买资产、借壳分别选择 DD 子集，不能机械套 IPO 全章。
- **高风险回退**：疑似借壳或规避重组上市时，应回退 IPO 标准 DD 和 ma-structure 深度论证。
...
| 日期 | 变更 |
|------|------|
| 2026-04-25 | 初版。锁定 6 阶段编排 + 阶段 2 按子类型分流（一般并购 / 重大资产重组 / 上市公司收购 / 借壳）+ 阶段 3 定向 DD 子集（必查 10 章 + 子类型追加）+ 借壳场景回退嵌套 wf-ipo-dd-full + 上市公司主导项目的信披自查必含 |
