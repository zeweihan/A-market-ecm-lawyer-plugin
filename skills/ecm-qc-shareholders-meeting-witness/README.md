...
3. **三种特殊表决方式专项强校验** — 基于《公司法》《上市公司股东会规
   则》《上市公司治理准则》等法规，对每条议案打"特别决议 / 累积投
   票 / 中小投资者单独计票"三个标签，再与通知标注、见证意见表述、
   表决结果格式做强校验（识别 → 核对 → 处理三步法）。
4. **9 类真实高频错误扫描** — 基础信息类、议案结构类、特别决议类
   （含新增高危项）、累积投票类（含新增高危项）、关联回避与单独计
   票类（含新增高危项）、模板残留类、章程适用类、底稿类、质控程序类。
5. **形式要件 14 项清单勾对** — 标题、声明段、结论措辞、签字律师、
   律所盖章等。
6. **严格的输出格式约束** — 修订者固定为可配置的名称（默认"内核"）；
...
├── SKILL.md                         # 主入口：硬规则清单 + 输出契约 + 工作流
├── LICENSE                          # MIT
├── README.md                        # 本文件
├── scripts/
│   ├── fetch_cninfo_announcements.py   # 巨潮公告拉取脚本
│   ├── render_paragraphs.py            # 重建 docx 每段渲染后编号
│   └── extract_images.py               # 提取 docx 内嵌图片（视觉校验）
└── references/
    ├── cross-check-matrix.md        # 交叉比对原则 + 12 项字段 + A+H 三会专项
    ├── voting-rules.md              # 三种特殊表决方式唯一定义处（特别决议 /
    │                                # 累积投票 / 单独计票 + 矩阵 X/Y/Z）
    ├── anti-misjudgment.md          # 批注前防误判（模板锚点 + 自动编号识别）
    ├── checklists.md                # 形式 14 项 + 规范用语 + 视觉校验 + 实质 8 项
    ├── common-errors.md             # A-I 九类常见错误库（含新增高危项）
    └── comment-templates.md         # 批注标准话术（唯一话术库）
```
...
- 《公司法》—— 特别决议（第 116 条）、累积投票制（第 119 条）的法律基础
- 《上市公司股东会规则》（2025 年修订，原《上市公司股东大会规则》）——
  特别决议事项、累积投票、中小投资者单独计票等（**具体条款号以现行有效版本
  为准**；引用时建议只引法规名称 + 事项，不强行标注修订年份，避免标错）
- 《上市公司治理准则》—— 累积投票制强制适用情形
- 上交所/深交所/北交所《股票上市规则》—— 特别决议事项扩展、单独计票
  要求
