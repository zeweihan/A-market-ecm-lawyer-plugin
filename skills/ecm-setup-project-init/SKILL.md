...
- **项目类型先行**：初始化前必须区分 IPO、再融资、并购重组、新三板、跨境或债券项目；类型不明时只建通用目录并列待确认项。
- **目录不得臆造**：新增目录必须能映射到项目阶段、底稿类别或输出文件；不因单个客户文件临时创造平行体系。
- **后续链路提示**：初始化报告必须给出下一步应触发的 setup / design / DD / research skill，避免项目启动后断流。
- **留痕要求**：项目简称、客户全称、项目类型、创建时间、创建人假设必须写入初始化报告，便于内核和归档复核。
