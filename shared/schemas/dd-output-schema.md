...
维护者: DD skill 侧 + ecm-draft 侧共同遵守
被哪些 skill 引用:
  ...
  下游（按本契约解析 DD Memo，拼接 / 提取）:
    ...
    - ecm-qc-work-report-review（反向定位到原 Memo 做审查）
本版编制日期: 2026-04-24
version: 1.0.0
---
...
| 字段 | 取值 | 说明 |
|------|------|------|
| `NN` | 01-17 | 对应 `shared/templates/project-folder-structure.md` 的**项目目录序号**，不是《编报规则第 12 号》的章节号 |
| `章节名` | 见下表 | 与目录名末段保持一致，避免出现空格 / 特殊字符 |
| `YYYYMMDD` | 8 位日期 | Memo 出具日期；同一 skill 多次出具时保留最近版本，老版归档到 `05-底稿和附件/DD-Memo-历史版本/` |

### skill → 目录 → 章节名 对照（硬契约）

| skill | 目录序号 NN | 编报章节号 | 目录 | 章节名（Memo 文件名） | 一级标题 |
|-------|------------|------------|------|--------------------|---------|
| `ecm-dd-approval` | 01 | 1 | `02-01-批准和授权/` | `批准和授权` | `# DD Memo：本次交易的批准和授权` |
| `ecm-dd-entity` | 02 | 2 | `02-02-主体资格/` | `主体资格` | `# DD Memo：发行人主体资格` |
| `ecm-dd-establishment` | 03 | 3 | `02-03-历史沿革/` | `设立` | `# DD Memo：发行人设立` |
| `ecm-dd-history` | 03 | 6 | `02-03-历史沿革/` | `历史沿革` | `# DD Memo：股本及其演变` |
| `ecm-dd-independence` | 04 | 4 | `02-04-独立性/` | `独立性` | `# DD Memo：发行人独立性` |
| `ecm-dd-shareholders` | 05 | 5 | `02-05-股东及实控人/` | `股东及实控人` | `# DD Memo：股东与实际控制人` |
| `ecm-dd-business` | 06 | 7 | `02-06-业务资质/` | `业务` | `# DD Memo：发行人业务` |
| `ecm-dd-related-party` | 07 | 8 | `02-07-关联交易与同业竞争/` | `关联交易与同业竞争` | `# DD Memo：关联交易与同业竞争` |
| `ecm-dd-assets` | 08 | 9 | `02-08-主要财产/` | `主要财产` | `# DD Memo：主要财产` |
| `ecm-dd-debt` | 09 | 10 | `02-09-重大债权债务/` | `重大债权债务` | `# DD Memo：重大债权债务` |
| `ecm-dd-charter` | 10 | 11 | `02-10-公司治理/` | `公司章程与治理` | `# DD Memo：公司章程与组织机构` |
| `ecm-dd-directors` | 11 | 12 | `02-11-董监高/` | `董监高` | `# DD Memo：董事、监事及高级管理人员` |
| `ecm-dd-tax` | 12 | 13 | `02-12-税务/` | `税务` | `# DD Memo：税务` |
| `ecm-dd-environment` | 13 | 14 | `02-13-环保与安全生产/` | `环保与安全生产` | `# DD Memo：环境保护、产品质量、安全生产` |
| `ecm-dd-fundraising` | 14 | 15 | `02-14-募集资金运用/` | `募集资金运用` | `# DD Memo：募集资金运用` |
| `ecm-dd-litigation` | 15 | 16 | `02-15-诉讼仲裁处罚/` | `诉讼仲裁处罚` | `# DD Memo：诉讼、仲裁、行政处罚` |
| `ecm-dd-compliance` | 16 | 17 | `02-16-其他合规事项/` | `其他合规事项` | `# DD Memo：其他合规事项` |
...
> **report-assembly 章节顺序**严格按本表"目录序号 NN"升序拼接（先设立后历史沿革的实务习惯由 NN=03 两个 skill 的小节内顺序体现，见 § 4）；DD Memo 顶部的 `chapter_no` 则按本表"编报章节号"填写。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.0.0 | 2026-04-24 | 首版。冻结：五段式二级标题、风险级别枚举、风险汇总表列定义、一级标题→章节号映射、Memo 落地路径模式 |
