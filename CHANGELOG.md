...
### Changed — 见证意见内核 skill 第四轮迭代：基于资深律师对 phase 2（8 份见证意见）内核结果的实测反馈（2026-06-16，`ecm-qc-shareholders-meeting-witness` v0.2.0 → v0.3.0）

本轮核心是**降低过度挑错（误判）、提升批注精准度与可读性**，让内核输出更稳定地替代人。逐条对应资深律师反馈：

- **减少误判（新增 `anti-misjudgment.md` 第三部分）**：
  - 董事/董秘"出席"、高管"列席"是《公司法》规范区分，**不再**误批"口径不一"（强瑞）。
  - 字号/字体/排版一致性文本不可靠判定，**禁止**输出此类批注、尤禁【必改】（东方盛虹"字号不一致"实为误判）。
  - **只选举 1 名董事/监事时累积投票不适用、不披露**，即使有 30% 控股股东也不触发——`voting-rules.md` §三新增"适用前提"、矩阵 Y 增加"应选=1 跳过"行、`common-errors.md` 新增 D11（众泰）。
  - "中小投资者"等定义的细微措辞差异（如"及其一致行动人"）默认不批，除非实际改变计票结果（众泰"看不懂的批注"）。
  - 精确区分会议通知/会议记录/会议决议；底稿内部无实质影响的细节默认不批（澄天伟业）。
- **删除过度/不必要要求**：表决程序段不再追加"留存计票监票人员名单及签字底稿"（众泰）；停用"董事/高管请假须出具书面请假条"要求（纵横，《股东会规则》未强制出席）——`common-errors.md` H8 停用、`comment-templates.md` §2.7/§10 调整、`checklists.md` §五加"克制原则"。
- **不臆断年份/外部事实（新增硬规则 16）**：批注不写"金杜与 Mallesons 分家"的具体年份（众泰）；不建议项目组给法规补注修订年份、体例不一至多【建议】统一为不标注（澄天伟业）。
- **提升漏判精度**：新增"公告/通知名称逐字核对实际披露标题"（强瑞"延期召开…通知" vs 实际"…通知（延期后）"）——`cross-check-matrix.md` 字段增至 13 项 + 场景 D、`common-errors.md` 新增 A10；从通知摘录回避名单须保留"等"字、开放式不得转写成封闭清单（东方盛虹）——场景 E + `common-errors.md` 新增 E13。
- **措辞校准**：中小投资者单独计票分母统一为律师范本"占出席会议中小投资者及中小投资者代理人有表决权股份总数的【】%"（信立泰）；签署日期不再硬性断言"应为股东会召开当日"，改为区间 + 会后据实补填（强瑞）。
- **软处理监事**：监事会撤销由【必改】+自动删除降级为【核实】+提示项目组结合公司实际治理结构核实（东方盛虹"个别公司没有取消监事"）——硬规则 7、`voting-rules.md` §四、`comment-templates.md` §3.6、`common-errors.md` E8/E12 同步。
- **批注两道闸（输出契约 §4 + 硬规则 19）**：每条批注须过"可读性闸（没看过原文也一眼看懂改什么）+ 相关性闸（影响见证结论才批）"，宁可少批边缘问题、不可错批规范写法。
- **成本与聚焦（SKILL.md Step 1.5）**：大体量扫描件深度逐页 OCR 核对有价值但 token 成本高，默认聚焦关键页、用户要求或发现异常时再扩展（纵横 45 页扫描件议案序号笔误）。
- **文档一致性修正**：`README.md` 法规依据移除与操作文件冲突且未能核验的《股东会规则》具体条款号，改为"以现行有效版本为准"。
